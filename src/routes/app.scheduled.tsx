...
function Scheduled() {
  ...
  const defaultWhen = useMemo(() => toLocalInputValue(new Date(Date.now() + 15 * 60000).toISOString()), []);
  ...
  useEffect(() => {
    if (!tenant) return;
    return onSnapshot(
      query(collection(db, "tenants", tenant.id, "scheduled_messages"), orderBy("scheduledAt", "asc")),
      (s) => setItems(s.docs.map((d) => ({ id: d.id, ...(d.data() as object) })) as Scheduled[]),
    );
  }, [tenant]);
  ...
  const create = async () => {
    ...
    if (num.length < 10) return toast.error("Número inválido (use DDI+DDD+número, ex: 5511999999999)");
    ...
    setSaving(true);
    try {
      await addDoc(collection(db, "tenants", tenant.id, "scheduled_messages"), {
        instanceName, number: num, text: text.trim(),
      ...
      setText(""); setNumber(""); setWhen(toLocalInputValue(new Date(Date.now() + 15 * 60000).toISOString()));
    } catch (e: any) {
  ...
  const remove = async (id: string) => {
    ...
    await deleteDoc(doc(db, "tenants", tenant.id, "scheduled_messages", id));
  };

  const statusBadge = (s: Scheduled["status"]) => {
    if (s === "sent") return <Badge variant="secondary" className="gap-1"><CheckCircle2 className="size-3" /> Enviado</Badge>;
    if (s === "failed") return <Badge variant="destructive" className="gap-1"><AlertCircle className="size-3" /> Falhou</Badge>;
    return <Badge variant="outline" className="gap-1"><Clock className="size-3" /> Pendente</Badge>;
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Envio futuro de mensagens via WhatsApp. Um cron processa pendências automaticamente.</p>
      </div>

      <div className="grid lg:grid-cols-[1fr_1.2fr] gap-5">
        {/* Form */}
        <div className="rounded-2xl border border-border bg-card/30 p-5 space-y-3">
          <h2 className="font-semibold text-sm flex items-center gap-2"><CalendarClock className="size-4" /> Novo agendamento</h2>
          ...
          <div>
            ...
            <Select value={instanceName} onValueChange={setInstanceName}>
              ...
              <SelectContent>
                {instances.length === 0
                  ? <div className="px-2 py-1 text-xs text-muted-foreground">Nenhuma instância conectada</div>
                  : instances.map((i) => <SelectItem key={i.id} value={i.name}>{i.name} · {i.status}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          ...
          <div>
            ...
            <Textarea rows={5} value={text} onChange={(e) => setText(e.target.value)} placeholder="Olá! Lembrando do nosso compromisso amanhã às 14h." />
          </div>
                ...
                <li key={m.id} className="py-3 flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <Badge variant="outline" className="text-[10px]">{m.instanceName}</Badge>
      ...
      <div className="rounded-2xl border border-dashed border-border p-4 text-xs text-muted-foreground">
        ...
        <p>Configure um serviço externo (cron-job.org, GitHub Actions, EasyCron) para chamar a cada minuto:</p>
        ...
        <p className="mt-1">Defina o segredo <code>CRON_SECRET</code> nas variáveis do servidor.</p>
      </div>
    </div>
  );
}
